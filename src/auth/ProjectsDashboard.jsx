...
export default function ProjectsDashboard({ user, onOpenProject, onLogout }) {
  ...
  return (
    <div style={{
      ...
      fontFamily: "Syne, sans-serif", color: "#faf8f4",
    }}>
      {/* Header */}
      <div style={{
        display: "flex", justifyContent: "space-between", alignItems: "center",
        padding: "20px 40px", borderBottom: "1px solid rgba(212, 176, 98, 0.1)",
      }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
          ...
          <span style={{
            ...
            color: "#d4b062", letterSpacing: 3, opacity: 0.6,
          }}>PRO</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
          ...
          <button onClick={onLogout} style={{
            padding: "8px 16px", borderRadius: 8, border: "1px solid rgba(212, 176, 98, 0.2)",
            background: "transparent", color: "#d4b062", fontSize: 12, cursor: "pointer",
            fontFamily: "Syne",
          }}>Déconnexion</button>
        </div>
      </div>
            ...
            {projects.map(p => (
              <div key={p.id}
                ...
                style={{
                  ...
                  background: "rgba(26, 23, 8, 0.5)", border: "1px solid rgba(212, 176, 98, 0.1)",
                  transition: "all 0.2s",
                }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = "rgba(212, 176, 98, 0.3)"; e.currentTarget.style.transform = "translateY(-2px)"; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(212, 176, 98, 0.1)"; e.currentTarget.style.transform = "translateY(0)"; }}
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div>
                    <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 4 }}>{p.name}</div>
                    {p.client_name && (
                      <div style={{ fontSize: 12, color: "rgba(255,255,255,0.4)" }}>Client: {p.client_name}</div>
                    )}
                  </div>
